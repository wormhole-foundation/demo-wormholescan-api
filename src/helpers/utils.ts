...
export function getRandomPlatform(token: NTTToken): { platform: string; address: string; chainId: number } | null {
  ...
  try {
    ...
    if (mappedPlatform) {
      try {
        ...
        console.warn(`Could not convert mapped platform ${mappedPlatform} to chain ID`);
        return null;
      }
    }
