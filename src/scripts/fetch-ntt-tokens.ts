...
async function fetchNTTTokens() {
  ...
  try {
    ...
    const tokensWithPlatforms = tokens.filter(token => Object.keys(token.platforms).length > 0);
    const tokensToProcess = tokensWithPlatforms.slice(0, TOKENS_TO_PROCESS);
    
    console.log(`🔍 Fetching detailed NTT information for first ${TOKENS_TO_PROCESS} tokens...\n`);
    ...
    for (const token of tokensToProcess) {
      ...
      if (!platformInfo) {
        console.error(`❌ Could not determine platform info for ${token.symbol}`);
        continue;
      }
      ...
      try {
        ...
        const detail = await wormholeScanAPI.get(`/ntt/token/${chainId}/${address}`) as NTTTokenDetail;
        ...
        console.log(`Transceiver Address: ${detail.home.manager.transceivers[0]?.address || 'N/A'}`);
        
      } catch (error) {
        console.log(`❌ No NTT configuration found for ${token.symbol} on ${platform}`);
      }
